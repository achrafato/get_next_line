...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/06 10:54:03 by aibn-che          #+#    #+#             */
/*   Updated: 2023/12/09 12:24:29 by aibn-che         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*check_validity(int fd, char **keeper)
{
	if (fd < 0 || fd > OPEN_MAX)
		return (NULL);
	else if (BUFFER_SIZE < 1 || read(fd, 0, 0) < 0
		|| (size_t)BUFFER_SIZE > INT_MAX)
		return (free(keeper[fd]), keeper[fd] = NULL, NULL);
	return ("1");
}
...
char	*get_next_line(int fd)
{
	...
	container = NULL;
	if (!check_validity(fd, keeper))
		return (NULL);
	if (!allocate_buffer(&buffer, &keeper[fd]))
		return (NULL);
	readed = read(fd, buffer, BUFFER_SIZE);
	while (readed > 0 || keeper[fd])
	{
		buffer[readed] = '\0';
		mingle_data(&container, &buffer, &keeper[fd]);
		if (!container)
		...
		readed = read(fd, buffer, BUFFER_SIZE);
		if (readed == -1)
			return (free(keeper[fd]), keeper[fd] = NULL, free(buffer), NULL);
	}
	free (buffer);
	return (container);
}
